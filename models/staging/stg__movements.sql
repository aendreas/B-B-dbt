WITH movements AS (
    SELECT CAST(event_type AS VARCHAR(11)) AS event_type,
    CAST(planned_timestamp AS VARCHAR(23)) AS planned_timestamp,
    CAST(actual_timestamp AS VARCHAR(23)) AS actual_timestamp,
    ...
    CAST(gbtt_timestamp AS VARCHAR(23)) AS gbtt_timestamp,
    ...
    FROM {{ source('movements', 'movements')}}
)
